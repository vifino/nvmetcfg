(import ./lib.nix) {
  ...
  in ''
    ...
    # Create the tcp port.
    target.succeed("nvmet port add 1 tcp [::]:4420")
    ...
    assert "ipv6" in target.succeed("cat /sys/kernel/config/nvmet/ports/1/addr_adrfam")
    assert "::" in target.succeed("cat /sys/kernel/config/nvmet/ports/1/addr_traddr")
